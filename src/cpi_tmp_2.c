...
void cpi_Initialize() {
    ...
    g_vec = calloc(1, sizeof(Vec));
    if (!g_vec) {
        tklog_critical("in cpi_Initialize(): calloc failed");
        return;
    }
    ...
    char** const dst_buffer)
{
    ...
    tklog_scope(*dst_buffer = (char*)calloc(1, file_size + 1));
    if (!*dst_buffer) {
        fclose(file);
    ...
    unsigned int* p_binding_stride) 
{
    ...
    tklog_scope(SpvReflectInterfaceVariable** input_vars = calloc(input_var_count, sizeof(SpvReflectInterfaceVariable*)));
    if (!input_vars) {
    ...
    tklog_scope(SDL_GPUVertexAttribute* attribute_descriptions = calloc(input_var_count, sizeof(SDL_GPUVertexAttribute)));
...
SDL_GPUShaderCreateInfo _cpi_Shader_CreateShaderInfo(const Uint8 *spv_code, size_t spv_code_size, const char *entrypoint, int is_vert)
{
    ...
    SpvReflectDescriptorBinding **bindings = calloc(binding_count, sizeof(SpvReflectDescriptorBinding*));
    if (!bindings) {
    ...
    bool enable_debug) 
{
    ...
    // spv code compilation
    {
        ...
        tklog_scope(shader.p_spv_code = calloc(1, shader.spv_code_size));
