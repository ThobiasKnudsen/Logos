...
CM_RES sdl3_window_show(const struct tsm_key* p_window_key, const struct tsm_key* p_graphics_pipeline_key) {
    ...
    while (running) {
        ...
        CM_SCOPE(SDL_BindGPUVertexBuffers(render_pass, 0, &buffer_binding, 1));
...
CM_RES sdl3_window_show_1(const struct tsm_key* p_window_key, const struct tsm_key* p_graphics_pipeline_key) {
    ...
    struct sdl3_graphics_pipeline* p_graphics_pipeline = caa_container_of(p_graphics_pipeline_base, struct sdl3_graphics_pipeline, base);
    CM_ASSERT(p_graphics_pipeline->p_graphics_pipeline);
    unsigned int rect_size = sizeof(Rect);
    SDL_GPUTransferBufferCreateInfo transfer_info = {
        .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD,
        .size = rect_size
    };
    SDL_GPUTransferBuffer* transfer_buffer = SDL_CreateGPUTransferBuffer(p_gpu_device, &transfer_info);
    CM_ASSERT(transfer_buffer);
    SDL_GPUBufferCreateInfo buffer_create_info = {
        .usage = SDL_GPU_BUFFERUSAGE_VERTEX,
        .size = rect_size
    };
    SDL_GPUBuffer* gpu_buffer = SDL_CreateGPUBuffer(p_gpu_device, &buffer_create_info);
    CM_ASSERT(gpu_buffer);

    // FPS constants for capping below 60 (e.g., 30 FPS)
    #define kTargetFPS 16
    ...
    while (running) {
        ...
        unsigned int tex_width = 0, tex_height = 0;
        CM_ASSERT(SDL_WaitAndAcquireGPUSwapchainTexture(cmd_buffer, p_window, &swapchain_tex, &tex_width, &tex_height));
        // Define single full-screen rectangle
        Rect rect = {
            .rect = { .x = 0.5f*(float)tex_width, .y = 0.5f*(float)tex_height, .w = (float)tex_width, .h = (float)tex_height },
            .rotation = 0.0f,
            .corner_radius_pixels = 0.f,
            .color = { .r = 255, .g = 255, .b = 255, .a = 255 },
            .tex_index = 0,
            .tex_rect = { .x = 0.0f, .y = 0.0f, .w = 1.0f, .h = 1.0f }
        };
        // Upload rect data
        void* mapped_data = SDL_MapGPUTransferBuffer(p_gpu_device, transfer_buffer, false);
        CM_ASSERT(mapped_data);
        memcpy(mapped_data, &rect, rect_size);
        SDL_UnmapGPUTransferBuffer(p_gpu_device, transfer_buffer);
        // Copy to GPU buffer
        SDL_GPUCopyPass* copy_pass = SDL_BeginGPUCopyPass(cmd_buffer);
        SDL_GPUTransferBufferLocation source = { transfer_buffer, 0 };
        SDL_GPUBufferRegion destination = { gpu_buffer, 0, rect_size };
        ...
        // Push dummy uniform data onto the command buffer (for vertex shader UBO)
        SDL_PushGPUVertexUniformData(cmd_buffer, 0, &dummyUBO, sizeof(dummyUBO));
        // Push time uniform for fragment shader
        SDL_PushGPUFragmentUniformData(cmd_buffer, 0, &time, sizeof(float));
        // Set up the color target info for the render pass.
        SDL_GPUColorTargetInfo color_target = {0};
        color_target.texture = swapchain_tex;
        color_target.mip_level = 0;
