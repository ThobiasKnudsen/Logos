...
static bool test_rcu_basic_operations(void) {
    CM_LOG_INFO("Testing basic RCU operations...");
...
static bool test_hash_table_operations(void) {
    ...
    if (node != NULL) {
        CM_LOG_ERROR("Lookup on empty table should return NULL");
        rcu_read_unlock();
        return false;
    }
    rcu_read_unlock();
    ...
    if (!test_node) {
        CM_LOG_ERROR("Failed to allocate test node");
        return false;
    }
    ...
    if (node == NULL) {
        CM_LOG_ERROR("Lookup should find the added node");
        rcu_read_unlock();
        return false;
    }
    ...
    if (found_node->value != 42 || strcmp(found_node->key, "test_key") != 0) {
        CM_LOG_ERROR("Found node has wrong data");
        rcu_read_unlock();
        return false;
    }
    rcu_read_unlock();
    ...
    if (!test_node2) {
        CM_LOG_ERROR("Failed to allocate second test node");
        return false;
    }
    ...
    if (result == &test_node2->lfht_node) {
        CM_LOG_ERROR("add_unique should fail for duplicate key");
        free(test_node2);
        return false;
    }
    ...
    if (original_node->value != 42) {
        CM_LOG_ERROR("add_unique should return original node");
        free(test_node2);
        return false;
    }
    ...
    rcu_read_unlock();
    if (count != 1) {
        CM_LOG_ERROR("Node count should be 1, got %lu", count);
        return false;
    }
    
    // Test deletion
    if (cds_lfht_del(test_ht, &test_node->lfht_node) != 0) {
        CM_LOG_ERROR("Failed to delete node");
        return false;
    }
    ...
    node = cds_lfht_iter_get_node(&iter);
    if (node != NULL) {
        CM_LOG_ERROR("Node should be deleted");
        rcu_read_unlock();
        return false;
    }
    rcu_read_unlock();
    ...
    CM_LOG_INFO("Hash table operations passed");
    return true;
}

/* Test error conditions */
static bool test_error_conditions(void) {
    CM_LOG_INFO("Testing error conditions...");
    ...
    rcu_read_lock();
    CM_LOG_INFO("Read lock without registration should have logged an error");
    
    // Test unregistration without registration - this should fail but not crash
    rcu_unregister_thread();
    CM_LOG_INFO("Unregistration without registration should have logged an error");
    ...
    test_ht = cds_lfht_new_flavor(16, 16, 1024, CDS_LFHT_AUTO_RESIZE, &urcu_memb_flavor, NULL);
    if (!test_ht) {
        CM_LOG_ERROR("Failed to create hash table for error testing");
    ...
    CM_LOG_INFO("Error condition tests completed");
    return true;
}
...
static bool test_thread_safety(void) {
    CM_LOG_INFO("Testing thread safety...");
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        if (pthread_create(&threads[i], NULL, test_thread_func, &thread_ids[i]) != 0) {
            CM_LOG_ERROR("Failed to create thread %d", i);
            return false;
        }
    }
    ...
    for (int i = 0; i < num_threads; i++) {
        if (pthread_join(threads[i], NULL) != 0) {
            CM_LOG_ERROR("Failed to join thread %d", i);
            return false;
        }
    }
    
    CM_LOG_INFO("Thread safety tests passed");
    return true;
}

/* Main test function */
bool test_urcu_safety_wrapper(void) {
    CM_LOG_INFO("Starting RCU/LFHT safety wrapper tests...");
    ...
    if (all_passed) {
        CM_LOG_INFO("All RCU/LFHT safety wrapper tests passed!");
    } else {
        CM_LOG_ERROR("Some RCU/LFHT safety wrapper tests failed!");
    }
...
int main(int argc, char *argv[]) {
    ...
    CM_LOG_INFO("RCU/LFHT Safety Wrapper Test Suite");
    CM_LOG_INFO("==================================");
    
    bool success = test_urcu_safety_wrapper();
    
    if (success) {
        CM_LOG_INFO("All tests passed successfully!");
        return 0;
    } else {
        CM_LOG_ERROR("Some tests failed!");
        return 1;
    }
} 