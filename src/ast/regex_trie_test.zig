...
// Import as modules (from build.zig)
const regex_trie = @import("regex_trie");
const RegexTrie = regex_trie.RegexTrie;
const RegexTrieError = regex_trie.RegexTrieError;

// Import the new verztable-based implementation
const regex_trie_2 = @import("regex_trie_2");
const RegexTrie_2 = regex_trie_2.RegexTrie_2;
const RegexTrieError_2 = regex_trie_2.RegexTrieError_2;
...
// ============================================================================
// RegexTrie_2 (Verztable-based) Benchmark Tests
// ============================================================================

test "RegexTrie_2.init" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();

    const trie = try RegexTrie_2.init(allocator);
    defer trie.deinit();

    try testing.expect(trie.leaf_value == null);
    try testing.expect(trie.children.count() == 0);
    try testing.expect(trie.regexes.items.len == 0);
    try testing.expect(trie.matcher_updated == true);
}

test "RegexTrie_2: insert and get simple literal" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();

    var root = try RegexTrie_2.init(allocator);
    defer root.deinit();
...
test "RegexTrie_2: insert multiple literals" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();
...
test "RegexTrie_2: insert regex pattern" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();
...
// Test inserting many words from words.txt with RegexTrie_2 (verztable-based)
test "regex_trie_2_many_words" {
    std.debug.print("\n=== Starting regex_trie_2_many_words test (VERZTABLE) ===\n", .{});
    ...
    std.debug.print("Starting word insertion (verztable)...\n", .{});
    var insert_timer = try std.time.Timer.start();
    _ = insert_timer.lap();
    ...
    std.debug.print("Inserted {d} words into trie (verztable)\n", .{num_words});
    std.debug.print("\n=== Benchmark (VERZTABLE): Insert Literal Strings ===\n", .{});
    std.debug.print("Inserted {d} words in {d:.2} ms ({d:.0} ops/sec)\n", .{ num_words, insert_elapsed_ms, insert_ops_per_sec });
    std.debug.print("Average: {d:.3} us per insert\n", .{insert_elapsed_ms * 1000.0 / @as(f64, @floatFromInt(num_words))});

    // Bulk verification
    std.debug.print("Starting bulk verification (verztable)...\n", .{});
    const words_file2 = try std.fs.cwd().openFile("testdata/words.txt", .{});
    defer words_file2.close();
    ...
    std.debug.print("\n=== Benchmark (VERZTABLE): Get Literal Strings ===\n", .{});
    std.debug.print("Performed {d} lookups in {d:.2} ms ({d:.0} ops/sec)\n", .{ max_verify_words, elapsed_ms, ops_per_sec });
    std.debug.print("Average: {d:.3} us per lookup\n", .{elapsed_ms * 1000.0 / @as(f64, @floatFromInt(max_verify_words))});
    ...
    std.debug.print("\n=== Benchmark (VERZTABLE): Destroy ===\n", .{});
    std.debug.print("Destroyed trie with {d} words in {d:.2} ms\n", .{ num_words, destroy_elapsed_ms });
    std.debug.print("Average: {d:.3} us per word\n", .{destroy_elapsed_ms * 1000.0 / @as(f64, @floatFromInt(num_words))});
}

test "benchmark_2: insert literal strings (verztable)" {
    const allocator = std.heap.c_allocator;
...
test "benchmark_2: get literal strings (verztable)" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();
