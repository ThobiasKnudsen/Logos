...
pub const RegexTrie = struct {
    ...
    fn updateMatcher(self: *RegexTrie) RegexTrieError!void {
        ...
        if (self.regexes.items.len == 0) {
            ...
            // First check if JIT is available
            var jit_available: c_uint = undefined;
            _ = pcre2_c.pcre2_config_8(pcre2_c.PCRE2_CONFIG_JIT, &jit_available);
            if (jit_available != 0) {
                if (regex.code) |code_ptr| {
                    // Use 0 to compile for all JIT options (like C++ version)
                    const jit_rc = pcre2_c.pcre2_jit_compile_8(@as(*pcre2_c.pcre2_code_8, @ptrCast(code_ptr)), 0);
                    if (jit_rc < 0) {
                        // JIT compilation failed, but continue without JIT
                        // Only warn if it's not an "unsupported" error
                        if (jit_rc != pcre2_c.PCRE2_ERROR_JIT_UNSUPPORTED) {
        ...
        // Match data is created on-demand in the get() function
    ...
    pub fn get(self: *RegexTrie, string: []const u8) RegexTrieError!MatchResult {
        ...
        while (pos < string.len) {
            ...
            if (idx != 255) {
                if (current.children.items[idx]) |child| {
                    // std.debug.print("Found literal child for '{c}', idx={d}, child={*}\n", .{ c, idx, child });
                    current = child;
                    // std.debug.print("After assignment, current is at {*}\n", .{current});
                    advance_len = 1;
                    advanced = true;
                }
            }
            ...
            if (!advanced) {
                ...
                if (current.compiled_regex) |*regex| {
                    ...
                    const match_result = regex.search(remaining, 0, null);
                    if (match_result) |*mr| {
                        defer @as(*pcrez.RegexMatch, @ptrCast(@constCast(mr))).deinit();
                        const start = mr.getStart(0) orelse break;
                        const end = mr.getEnd(0) orelse break;
                        if (start == 0 and end > 0) {
                            const regex_len = end;
                            // Identify which alternative matched (check capture groups 1..n)
                            var which: ?usize = null;
                            for (current.regexes.items, 1..) |_, g| {
                                const group_start = mr.getStart(g) orelse continue;
                                const group_end = mr.getEnd(g) orelse continue;
                                if (group_start != std.math.maxInt(usize) and group_end > 0) {
            ...
            // std.debug.print("Checking EOW for current at {*}, pos={d}\n", .{ current, pos });
            const eow_idx = current.child_indices[0];
            if (eow_idx != 255) {
        ...
        while (stack.items.len > 0) {
            ...
            if (current.regexes.items.len > 0) {
                ...
                if (current.compiled_regex) |*regex| {
                    ...
                    if (match_result) |*mr| {
                        defer @as(*pcrez.RegexMatch, @ptrCast(@constCast(mr))).deinit();
                        const start = mr.getStart(0) orelse continue;
                        const end = mr.getEnd(0) orelse continue;
                        if (start == 0 and end > 0) {
                            const regex_len = end;
                            ...
                            for (current.regexes.items, 1..) |entry, g| {
                                ...
                                if (group_start != std.math.maxInt(usize) and group_end > 0) {
                                    ...
                                    // Note: We could continue to check other alternatives too,
                                    // but PCRE2 typically only matches one alternative in an alternation
                                    // For true "all matches", we'd need to try each regex individually
                                }
                            }
                        }
                    }
